...
banks =[
  ...
  constructor(private http: HttpClient, private usersService: UsersService) { 
    this.banks.forEach((bank, index) => {
      this.bankList.push(new Bank((index + 1).toString(), bank.nameEn, bank.nameAr, 'active', bank.bic));
    });
  }
    ...
    this.usersService.getAccountData(this.toAccount).subscribe((res: any) => {
      if (res.status) {
        this.pickedAccountData={
          ...
          bank:this.bankList?.filter((bank: any) => bank.id == res.response?.filter((res: any) => res.kyc_detail_id == 121)[0]?.value)[0]?.nameAr||''
        }
      }
    })
  }
