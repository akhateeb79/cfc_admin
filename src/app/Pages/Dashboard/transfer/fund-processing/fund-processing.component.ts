...
@Component({
  selector: 'app-fund-processing',
  templateUrl: './fund-processing.component.html',
  styleUrls: ['./fund-processing.component.css']
})
export class FundProcessingComponent implements OnInit {
  ...
  error: string = "";
  info: any = null;
  fromAccounts: any = ["0190095517580024","0190095517580017"];
  toAccounts: any = ["0108095517580016"];
  amount: any = 0;
  userList: any[] = [];
  ...
  @Output() requestDataChange: EventEmitter<any> = new EventEmitter<any>(); // New output property
  ...
  this.requestNumber = Date.now();
    this.kycService.addWorkflowInstance(this.itemId, this.userId, this.requestNumber, 'initia', status, {
      ...
      (response) => {
        ...
        this.requestDataChange.emit(response); // Emit the response or updated data
      },
      (error) => {
